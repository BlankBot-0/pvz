...
func TestCreateReception_UnexpectedRole(t *testing.T) {
	t.Parallel()

	ctx := auth.SetUserRoleToCtx(context.Background(), moderatorRole)

	request := &pvzpb.CreateReceptionRequest{PvzId: "1"}

	service, _, _ := newService(t)

	_, grpcErr := service.CreateReception(ctx, request)
...
func TestCreateReception_Ok(t *testing.T) {
	t.Parallel()
	...
	pvzMock.CreateReceptionMock.Set(func(_ context.Context, _ string) (_ models.Reception, _ error) {
		return reception, nil
	})

	response, err := service.CreateReception(ctx, request)
	if err != nil {
...
func TestCreateReception_Fail(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:    "another reception in progress",
			mockErr: pvz.ErrAnotherReceptionInProgress,
